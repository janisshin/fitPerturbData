...
from banana import evaluate_banana
...
def calculateFitness(population, groundTruth):
    ...
    """
    scores = {}
    # groundTruthData = evaluate_banana.runBanana(groundTruth)
    for individual in os.listdir(population):
        f = open(population + "/" + individual, "r")
        individualData = evaluate_banana.runBanana(f.read())
...
def extractParams(population, parameters, groundTruth, folderName=''):
    ...
    for individual in os.listdir(population):
        ...
        individualData = evaluate_banana.runBanana(individualModel)
        
        scoreFile.write(str(individualData) + "\n")
        for k in parameters:
...
def runGeneticAlgorithm(population, groundTruth=models_banana.groundTruth_e, parameters=models_banana.K_LIST, lastGeneration=100, survivorRatio=(2,9), tolerance=1, runID=''):
    """
